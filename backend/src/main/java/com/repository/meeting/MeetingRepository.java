...
public interface MeetingRepository extends JpaRepository<Meeting, Integer> {
    ...
    @Query(value = "SELECT meeting.* FROM meeting JOIN invite USING(meeting_id)" +
            " WHERE room_id = :room_id AND meeting_id != :meeting_id",
            nativeQuery = true)
    List<Meeting> findAllByRoomId(@Param("room_id")Integer roomId, @Param("meeting_id") Integer meetingId);

    @Query(value = "SELECT meeting.* FROM meeting JOIN invite USING(meeting_id)" +
            " WHERE employee_id = :employee_id AND meeting_id != :meeting_id AND invite_accept = 1",
            nativeQuery = true)
    List<Meeting> findAllEmployeeMeetings(@Param("employee_id") Integer employeeId, @Param("meeting_id") Integer meetingId);
